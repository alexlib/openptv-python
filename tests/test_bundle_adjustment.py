...
from openptv_python.parameters import ControlPar, OrientPar
...
class TestBundleAdjustment(unittest.TestCase):
    ...
    def lightly_perturb_calibrations(self, true_cals: list[Calibration]) -> list[Calibration]:
        perturbed = [self.clone_calibration(cal) for cal in true_cals]
        deltas = [
            (np.array([0.0, 0.0, 0.0]), np.array([0.0, 0.0, 0.0])),
    ...
    def test_cavity_reprojection_improves(self):
        ...
        after_rms = reprojection_rms(observed_pixels, refined_points, refined_cals, control)
