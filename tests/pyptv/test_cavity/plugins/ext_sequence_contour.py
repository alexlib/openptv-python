...
from pyptv._backend import Frame, correspondences, MatchedCoords, default_naming, point_positions
...
class Sequence:
    ...
    def do_sequence(self):
        ...
        for frame in range(first_frame, last_frame + 1):
            ...
            # Corresp. + positions.
            frame = Frame(num_cams)
            for cam in range(num_cams):
                frame.targets[cam] = detections[cam]
                frame.num_targets[cam] = len(detections[cam])

            sorted_pos, sorted_corresp, _ = correspondences(
                frame,
                [coord.coords if hasattr(coord, "coords") else coord for coord in corrected],
                vpar,
                cpar,
                cals,
                [0] * num_cams,
